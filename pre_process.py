...
def prep(libri,out_dir=c.OUTPUT_DIR,name='0'):
    start_time = time()
    i=0
    ...
    for i in range(len(libri)):
        orig_time = time()
        filename = libri[i:i+1]['filename'].values[0].replace('\\', '/')
        _ ,_, _, name, _, file = filename.split("/")
        file = file.split('.')[0]
        # target_filename = out_dir + filename.split('/')[-1].split('.')[0] + '.npy'
        target_filename = f'{out_dir}/{name}-{file}.npy'
...
def preprocess_and_save(wav_dir=c.DATASET_DIR,out_dir=c.OUTPUT_DIR):
